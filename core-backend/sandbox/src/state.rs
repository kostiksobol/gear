...
use gear_sandbox::default_executor::{Caller as DefaultCallerStore, Store as DefaultExecutorStore};

pub(crate) type HostState<Ext> = Option<State<Ext>>;

pub(crate) type Store<Ext> = DefaultExecutorStore<HostState<Ext>>;

pub(crate) type Caller<'a, Ext> = DefaultCallerStore<'a, HostState<Ext>>;
